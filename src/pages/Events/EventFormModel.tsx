...
import { X, User, Phone, Calendar, MapPin, Globe, CheckCircle, ArrowRight, Ticket } from "lucide-react";
...
const EventFormModel = ({ eventName, categoryName }: Props) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      await axios.post(
        `${import.meta.env.VITE_API_URL}/api/eventApplications`,
        payload,
      );
        ...
        {!submitted && (
          <>
            ...
            <div className="bg-gradient-to-r from-[#83261D] to-[#B45F4A] px-4 sm:px-5 md:px-6 py-3 sm:py-3.5 md:py-4 sticky top-0 z-10 flex-shrink-0 relative">
              ...
              <DialogHeader>
                <DialogTitle className="flex flex-col gap-0.5 sm:gap-1 pr-8">
                  ...
                  <span className="text-xs sm:text-sm text-white/80 font-normal line-clamp-1">{eventName}</span>
                  <span className="text-[10px] sm:text-xs text-amber-200 font-medium line-clamp-1">{categoryName}</span>
                </DialogTitle>
              </DialogHeader>
            </div>
          ...
          <div className="flex-1 overflow-y-auto flex items-center justify-center">
            <div className="flex flex-col items-center justify-center py-10 sm:py-12 md:py-14 lg:py-16 px-4 sm:px-6 md:px-8 text-center">
              ...
              <p className="text-xs sm:text-sm md:text-base text-gray-500 mb-4 sm:mb-5 md:mb-6 max-w-[250px] sm:max-w-sm">
                Thank you for registering for {eventName}. Our team will contact you with further details soon.
              </p>
              <button
                onClick={() => setOpen(false)}
